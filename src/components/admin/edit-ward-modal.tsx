...
interface EditWardModalProps {
  ...
  ward: WardData | null
  onSave: (data: WardData) => void
}

export function EditWardModal({ open, onOpenChange, ward, onSave }: EditWardModalProps) {
  const [name, setName] = useState(ward?.name ?? "")
  const [floor, setFloor] = useState(ward?.floor ?? "")
  const [active, setActive] = useState(ward?.status !== "Inactive")

  function handleSubmit() {
    if (!ward || !name) return
    onSave({ ...ward, name, floor, status: active ? "Active" : "Inactive" })
    onOpenChange(false)
  }
  ...
  return (
    <Dialog key={ward?.id} open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        <DialogHeader className="px-8 pt-7 pb-0 gap-1">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-[14px] text-[#9ca3af] mt-1">Update ward information</p>
            </div>
            <button
